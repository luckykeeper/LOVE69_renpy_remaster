...
label scene21:
    ...
    lion "啊，就是这里，那家店（L:这是前面在摩天轮上提到的里昂要借的那家店铺，例のお店的这个表达很特殊，比如例のプール(那个泳池)、例のコンビニ(那个便利店)、例の電車(那个电车)等等，可以参考BV1i64y1e7N9）"

    # 莲 「ぬお…こは…」
    lian "这是……"

    # nil 「リオンと結ばれた日の翌々日。俺はリオンに連れられて『店舗を貸してくれる予定だったお店』にやってきた。」
    "和里昂结合的第三天。我被里昂带到了『预定租借店铺给她的那家店』"
    ...
    lion "呼啊——……呼啊——……啊呜——……感觉不大得劲啊……"

    # 莲 「なんだ、ウンコでも漏れそうなのか」
    lian "什么呀，大便都快漏出来了吗？"

    # 里昂 「そうそう、一週間溜まったのがついに…ってばか！緊張とかそういうのだよ！」
    lion "对对，积攒了一个星期终于……真是笨蛋啊！紧张什么的！"
    ...
    # 店长 「俺たちを出迎えてくれたのはどこかで見覚えのある、黒髪で長身の女性だった。」
    dinerowner "迎接我们的是似乎在哪里见过的，黑发高个子的女性"
    ...
    dinerowner "是啊。大概两周没见了吧。我知道你来这里的理由，打招呼就不用了，总之先坐下吧"
    ...
    dinerowner "为了缓解她的紧张，可乐和冰块。真是不错的选择"
    ...
    dinerowner "哈哈，我开玩笑的。来，今天的蛋糕。加了覆盆子酱做的巧克力蛋糕（L:覆盆子就是一般意义上说的树莓，国内又叫红桑子、野草莓、蛇头莓、桑莓等，一周目这款蛋糕也有出现）"
    ...
    dinerowner "但是只有你们两个人吃饭，不是很寂寞吗？"

    # 里昂 「あ、あの…！」
    lion "啊，那个…！"

    # nil 「まぁまぁ。まずは食べましょう。美味しいですよ？」
    "嘛嘛，先来吃吧，很好吃吧？"
    ...
    dinerowner "怎么样？我的球路多吧（L:原文球種，是棒球用语，指棒球投手投出的球路的类型和名称，有多种）"
    ...
    dinerowner "好——里昂酱，从刚才开始就没吃东西，你没事吧？"
    ...
    lion "那个…那个…我……没能实现我的约定…对不起…"

    # 店长 「あ、それでそんな落ち込んでたんですね。いんですよ、貴方が選んだ結果ですから。その分彼氏さんを幸せにしてあげて下さいね」
    dinerowner "啊，所以你才这么沮丧。没关系的哦，这是你自己选择的结果，希望你的男朋友能够幸福"
    ...
    dinerowner "实现了自己的爱情，把店借给我吧。这种甜言蜜语是行不通的"
    ...
    lion "谢谢…对不起……呃，雾叶小姐……我想…我希望能再得到一次机会……"

    # 店长 「チャンスを…？　次はどんな恋を叶えるつもりですか？多分、ラブポーション・シクスティナインはもう作れないでしょうし」
    dinerowner "Chance……？接下来你打算实现什么样的恋爱呢？大概，你已经再也做不出LOVEPOTION・SIXTYNINE了"

    # 莲 「…どういう事だ？」
    lian "…这是怎么回事？"

    # 店长 「おや、知らなかったのですね。あのアイスは元々、リオンちゃんの『蓮くんへの恋心』が込められてるんですよ。今はそれが満たされてしまっているから…」
    dinerowner "啊，你不知道啊。那个冰淇淋原本就包含了里昂的『对莲君的恋慕之情』，但是现在已经得到满足了"

    # 店长 「同じものを作る事はできない。それに、作れたとしても、世界の運命すらねじ曲げてしまうような、強力な物は作れないでしょうね」
    dinerowner "所以不可能制造出同样的东西。而且，就算制造出来了，也不可能制造出能扭曲世界的命运这样强大的东西吧"
    ...
    # 店长 「確かに察しのい彼氏ですね…」
    dinerowner "的确是个敏感的男朋友……"

    # 里昂 「あとは…私の力で、一人でも多くの人を…幸せにしたいから…！」
    lion "嗯，是的，我自己也有点不在状态……"
    ...
    liu "别这样！这是刚从洗衣店出来的西装！要是碰上那种东西，不是会留下粉红色的标记吗！"
    ...
    dinerowner "啊，是啊。真是的，不要在重要的地方打断我的话。来，我给你可乐"
    ...
    dinerowner "姑且，你能告诉我你的想法吗？"

    # 里昂 「はい…えっと…」
    lion "那个……呃……"

    # 想瑠 「ねーねー蓋あかないんだけど。栓抜きちょうだい」
    liu "呐~呐~我打不开，把开瓶器给我一哈"
    ...
    lion "一开始，我和莲君结合的时候，也觉得就算放弃梦想也没关系"

    # 里昂 「でも、蓮くんに励まされて…そして、一緒にやってきた事全ても無駄になってしまうのも嫌で…」
    lion "但是，在莲君的鼓励下……而且，我也不想让一起经历的一切都白费……"

    # 里昂 「それに、蓮くんが自分のせいで私が夢を諦めたとは思って欲しくない…」
    lion "而且，我不希望莲君认为我因为他而放弃了梦想……"

    # 里昂 「そして、彼が応援してくれる夢を…一緒に叶えたくて…！」
    lion "而且，我希望他支持着我的梦想……一起来实现它……！"

    # 店长 「…全部彼氏の話ですね」
    dinerowner "……全部都是男朋友的事呢"
    ...
    dinerowner "世界啦，幸福啦，这种规模宏大的理论根本不合我的性格。也许你有这个能力，就算不是冰淇淋店，也可以改变整个世界"
    ...
    dinerowner "就是这样，里昂酱，莲君。在五天之内做出只有两个人才能做的冰淇淋。当然必须是好吃的"

    # 店长 「それがクリアできるなら、お話をしましょうか」
    dinerowner "如果可以搞定的话，我们再来谈谈吧"
    ...
    lion "哈啊……哈啊……没时间了……我是笨蛋……！"

    # 莲 「そう落ち込むなよ。頑張ってやってるじゃねぇか」
    lian "别那么消沉，你已经尽力了"

    # 里昂 「でも、結果が出ないんじゃ意味ないよ！」
    lion "但是，如果没有结果，那就没有意义了！"

    # nil 「リオンは苛立って語気を強める。」
    "里昂焦躁地加强语气"

    # 里昂 「しかし、自分の発言の意味に気づいて言葉を閉ざした。」
    lion "但是，注意到自己的发言的意思，就闭上了嘴"

    # 里昂 「ごめん…手伝って貰ってるのに、意味ないとか…言っちゃだめだよね…」
    lion "对不起……有人帮忙，不能说是没有意义……"
    ...
    lion "啊——！是、是的啊！好了，迈克尔，去吧，去玩吧！谢谢你为我做的一切！"
    ...
    lion "这是……不，这不可能是……总之，莲君！抓住牢固的东西，快点！"
    ...
    lion "“嗯…嘛，在给别人吃之前自己没尝过的话……"

    # 莲 「ていうか人に食べさせるのか…」
    lian "话说这个是要给别人吃啊…"

    # 里昂 「でも、もう時間もネタもないよ！　これに賭けるしかない！」
    lion "但是，已经没有时间和素材了！只能赌在这个上面了！"
    ...
    lion "Batch Koi！3——2！（L:前半原文ばっちこーい！是棒球用语，是自我激励用语）"
